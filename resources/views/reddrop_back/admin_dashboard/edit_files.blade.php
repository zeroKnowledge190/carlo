<main class="hic-documents">
    ...
    <li class="breadcrumb-item active">Hi, {{ Auth::user()->user_firstname }} {{ Auth::user()->user_lastname }} , Edit Files</li>
</ol>
    ...
    @endif
<b>Manage Advertisement</b>
<form enctype="multipart/form-data" action="/save-edit-files/{{ $editFiles->file_id_no }}" method="post">
<div class="row">
    <div class="col-md-3">
        <label>Company</label><br>
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" class='form-control' value="{{ $editFiles->file_id_no }}" name="file_id_no">
    </div>
    <div class="col-md-3">
        <label>File Name</label><br>
        <b>{{ $editFiles->file_name }}</b>
    </div>
</div>
<div class="row">
    <div class="col-md-3">
        <label>Created By</label><br>
        <b>{{ $editFiles->created_by }}</b>
    </div>
    <div class="col-md-3">
        <label>File Status</label><br>
        <select class="form-control" name="file_status">
            <option value="{{ $editFiles->file_status }}">{{ $editFiles->file_status }}</option>
